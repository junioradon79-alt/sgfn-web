-- Passeport parcelle v1 (Phase 1 point 2 du Document Directeur) — assemblage
-- de briques déjà existantes : on enrichit verifier_attestation() et la
-- branche certificat_vente de verifier_document() avec les champs qu'il
-- manquait pour construire un écran complet par lot (identifiant, localisation,
-- géométrie floutée, compteur de vérifications), sans nouvelle fonction ni
-- nouveau grant. Le score de confiance est déjà présent depuis ce matin.
...
-- verifier_document() : même enrichissement sur la branche certificat_vente
