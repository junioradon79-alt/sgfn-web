...
// Webhook CinetPay — pas de JWT (appelé par CinetPay directement)
// Sécurité : on vérifie le paiement via l'API CinetPay avant toute mise à jour
...
Deno.serve(async (req) => {
  ...
  // Trouver le paiement via reference_externe = 'cinetpay:<txId>'
  ...
  if (!paiement) {
    // Tentative par ID direct (transaction_id = paiement.id sans tirets)
    return new Response("paiement introuvable", { status: 404 });
  }

  if (paiement.statut === "confirme" || paiement.statut === "en_attente_validation") {
  ...
  if (accepted) {
    // Delegue a la RPC : bascule en 'confirme' ou 'en_attente_validation' selon le seuil configure
    const { error } = await admin.rpc("marquer_paiement_recu", {
      p_paiement_id: paiement.id,
      p_reference_externe: cpmPayId,
    });
    if (error) {
      console.error("Erreur marquer_paiement_recu", error);
      return new Response("Erreur interne", { status: 500 });
    }
  } else {
    await admin
      .from("paiements")
      .update({ statut: "echoue", reference_externe: cpmPayId })
      .eq("id", paiement.id);
  }
