...
- **Reste ouvert :** le **Site Vitrine** du handoff (8e maquette) n'est pas fait — `home`/`login` portent des couleurs figées, ce sera une reconstruction, pas une migration. Marque **SGNF conservée** dans la coquille (tranché avec le user) alors que le handoff affiche « SGFN ». Dette préexistante non touchée : erreur eslint dans `src/app/dashboard/lots/page.tsx` (setState synchrone dans un effet), datant de `e150470`.
