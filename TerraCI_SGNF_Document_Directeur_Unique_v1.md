...
**Version 1.6 — 10 juillet 2026 (soir)**
...
- **Module « Opérateur de saisie » (maker-checker) — développé et vérifié e2e en local le 10/07, ⚠️ PAS ENCORE DÉPLOYÉ EN PROD (voir dette #12).** Nouveau rôle `groupe_utilisateur.operateur_saisie` dédié à la mise à jour de la base (attributions + création de lotissements/îlots/lots), soit à la main, soit par import Excel. Workflow de **double validation** : l'opérateur soumet un diff résolu (RPC `soumettre_saisie` → table `soumissions_saisie`, file + journal), **un admin approuve** (`approuver_soumission`, applique réellement avec historique préservé et garde-fou anti-attestation via GUC `sgnf.skip_free_attestation`) **ou rejette** (`rejeter_soumission` + motif). Front `/dashboard/saisie` : l'opérateur ne voit que ce module, aperçu du diff classé avant soumission ; l'admin a une file de validation + **pastille de rappel** au menu. Étapes 1 (DB), 2 (saisie manuelle) et 3 (validation admin) livrées et committées (`72b633e`/`7d56d21`/`57cc2f3`), testées e2e au navigateur. Reste : import Excel (template simplifié + parsing SheetJS) et création de structure via formulaire. Généralise le travail manuel des Guides de Répartition.
...
### 4.3 Dette technique et risques ouverts (au 10/07)
...
| 12 | **Module Opérateur de saisie — à déployer + compléter** | 🟡 | Étapes 1-3 (DB + saisie manuelle + validation admin) codées, testées e2e en local, **pas encore déployées en prod** (tar.gz à refaire). Reste : étape 4 (import Excel — template + parsing SheetJS côté navigateur) et étape 5 (création de structure via formulaire). Compte test `manuel.operateur-saisie@sgfn.ci` créé. |
...
- Vérifié **e2e au navigateur** : cycle complet soumettre → valider → approuver (appliqué en base réelle puis revert) + soumettre → rejeter (motif stocké, 0 changement) ; **compte test `manuel.operateur-saisie@sgfn.ci` créé** (insert `auth.users`/`identities` direct, groupe posé par `handle_new_user`) → sidebar réduite au seul module, pas de badge (maker). ⚠️ **Module pas encore déployé en prod** — dette #12. Reste : étapes 4 (import Excel) et 5 (création de structure).
